...
const sendInviteEmail = (user, groupId) => {
	const emailButtonStyles = "display: inherit; margin-top: 15px; margin-left: auto; margin-right: auto; color: white; background-color: black; border-style: none; width: 85px; height: 25px;" 
	...
	const msg = {
		...
		html:
			`<strong>and easy to do anywhere, even with Node.js</strong><button type="button" style="${emailButtonStyles}"><a style="decoration: none; color: white;" href="${emailHref}>JOIN</a></button>`,
	};
	sgMail
		.send(msg)
		.then((res) => {
